...
router.post(
	'/create',
	[auth,
		check('detail_text').escape()],
		check('title', 'Минимальная длина 10 символов').isLength({min:10}).trim(),
		check('description', 'Минимальная длина 50 символов').isLength({min:50}).trim(),
	async (req, res) => {
		try {
			...
			if (!errors.isEmpty()) {
				return res.status(400).json({
					errors: errors.array(),
					message: 'Некорректные данные при создании тренинга',
				});
			}
