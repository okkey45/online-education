...
router.post('/create',
	[auth, check('stud_response', 'Напишите ответ, минимум 20 символов').isLength({min:20}).trim(),
	 check('creation_date').toDate()] ,
	 async (req, res) => {
	try {
		...
		if (!errors.isEmpty()) {
			return res.status(400).json({
				errors: errors.array(),
				message: 'Некорректные данные при ответе',
			});
		}
...
router.put('/edit/:id',
	[auth, check('prof_response', 'Напишите ответ, минимум 20 символов').isLength({min:20}).trim(),
	 check('score', 'Выставите оценку').notEmpty()] ,
	 async (req, res) => {
	try {
		const errors = validationResult(req);
